...
// FIREBASE & SEGURANÇA (Fase 1)
import { db } from './firebase';
import { collection, query, orderBy, onSnapshot, doc } from 'firebase/firestore';
import { useAuth } from './context/AuthContext'; // Novo motor de segurança
...
const App: React.FC = () => {
  ...
  // 1. ESCUTAR SERVIÇOS (Firestore Path: businesses/{ID}/services)
  // Mantido aqui para alimentar a Landing Page pública
  useEffect(() => {
    const q = query(
  ...
  // 2. ESCUTAR METADADOS (Redes, Galeria, Mapa)
  useEffect(() => {
  ...
  // Se o AuthContext ainda estiver a carregar o perfil, o Provider já mostra o loader.
  // Aqui gerimos apenas a transição de visualização entre Site e Dashboard.
  ...
  return (
    <div className="min-h-screen flex flex-col selection:bg-primary-light selection:text-primary-dark bg-brand-bg font-sans">
      ...
      {/* COMPONENTES DE LOGIN (Apenas se não estiver logado) */}
      ...
      {/* 
          DASHBOARD ADMINISTRATIVO (Fase 1: Multi-utilizador)
          Agora controlado pela presença do 'user' no contexto global.
          O AdminDashboard passará a receber o perfil (userData) para decidir o que mostrar.
      */}
      {user && (
      ...
      {/* 
          Nota: Mantemos o site visível "por baixo" ou escondido 
          se o dashboard estiver aberto para otimização de performance.
      */}
